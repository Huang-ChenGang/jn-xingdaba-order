...
public class QuoteDiscountServiceImpl extends AbstractQuote {

    public QuoteDiscountServiceImpl(@Qualifier("jnRestTemplate") RestTemplate jnRestTemplate,
    ...
    public String quote(QuoteRequestData requestData) {
        log.info("discount quote requestData: {}", requestData);
        // 获取报价参数
        ...
        for (QuoteBusTypeRequestData busType : busTypeList) {
            ...
            // *折扣优惠费率
            if (effectiveMultiplier(busPrice.getDiscountRates())) {
                busTypeDayQuote.setQuoteAmount(busTypeDayQuote.getQuoteAmount().multiply(busPrice.getDiscountRates()));
            }
